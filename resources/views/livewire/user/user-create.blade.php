                    ...
                    <form wire:submit.prevent="saveUser">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="mt-1 grid grid-cols-1 md:grid-cols-3 gap-2">
                                ...
                                @empty
                                    <p class="text-gray-500">No hay roles disponibles. Asegúrate de ejecutar los seeders.</p>
                                @endforelse
                            </div>
                            @error('selectedRole') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>

                        {{-- Campo para asignar supervisor, visible solo para Administradores y cuando el rol seleccionado es Conductor/Operador --}}
                        @if(auth()->user()->hasRole('Administrador') && $selectedRole == $conductorRoleId)
                            <div class="mb-4">
                                <label for="supervisor_id" class="block text-sm font-medium text-gray-700">Asignar Supervisor (Opcional)</label>
